...
將官網對外主軸從「LINE 群組 AI 翻譯／多語溝通助手」收斂為「LINE 多國翻譯、AI 摘要、每日報表與後台搜尋統計」。萬語通不只賣翻譯，而是協助雇主在 LINE 群組裡完成多國翻譯、語音、圖片 OCR、AI 摘要、每日報表與後台紀錄查詢。
...
- `join.html`
  - 加入頁 meta、首屏、證明點、動態中英文文案改為「翻譯、摘要、報表、搜尋統計」流程。
- `blog.html`
  - 部落格 meta、JSON-LD、首屏中英文改為 LINE 多國翻譯、AI 摘要、每日報表與後台搜尋統計知識庫。
- `README.md`
  - 新增本交接文件入口。

## 對外文案準則

- 優先使用：LINE 多國翻譯、語音翻譯、圖片 OCR、AI 摘要、每日報表、後台搜尋統計、報表匯出。
- 避免主標再使用：LINE 群組 AI 翻譯、多語溝通助手、只是翻譯 App。
- 翻譯引擎口徑：DeepL 專業翻譯引擎；不要再提雙引擎、自動路由或未啟用的外部翻譯方案。
